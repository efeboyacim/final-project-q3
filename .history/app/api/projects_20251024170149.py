...
def download_document(
    ...
    key = f"project-docs/{doc.project_id}/{doc.name}"
    try:
...
def update_document(
    document_id: int,
    file: UploadFile | None = File(None),   # yeni içerik (opsiyonel)
    new_name: str | None = Form(None),      # yeni ad (opsiyonel)
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    doc = db.query(Document).filter(Document.id==document_id).first()
...
def delete_document(
    ...
    # S3’te yoksa hata yapma; DB’den yine de sil
    try:
